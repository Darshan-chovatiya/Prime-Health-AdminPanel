  ...
  const handleUpdateService = async (serviceData: any) => {
    try {
      ...
      if (response.status === 200 && response.data) {
        ...
        const errorMessage = response.message || 'Failed to update service. Please try again.';
      ...
      if (error?.message) {
        ...
        if (error.message !== 'Request failed' && !error.message.includes('Request failed with status')) {
          ...
          if (errorMessage === 'Failed to update service. Please try again.') {
            errorMessage = 'Unable to update service. The service may be assigned to categories. Please remove it from all categories first.';
          }
        }
      } 
